...
1. **MCP tools** (when ` + "`" + `heydb serve` + "`" + ` is running) — structured queries for
   precise lookups, searches, and annotation management. Prefer this for
   accurate, up-to-date schema information.
2. **Generated docs** in ` + "`" + `.heydb/*.md` + "`" + ` — generated by ` + "`" + `heydb docs` + "`" + `, includes
   annotations with author and date. Read these directly for broad offline context.

Use MCP tools for targeted queries (specific table details, searching across
tables, or reading/writing annotations). Use the docs for broad context.
...
Annotations are accumulative — multiple annotations per entity are allowed.
Each annotation records the author and timestamp so you know who wrote what
and when. They survive ` + "`" + `heydb sync` + "`" + ` runs — never overwritten by introspection.
...
To share annotations with your team: run ` + "`" + `heydb push` + "`" + ` to export them as a
